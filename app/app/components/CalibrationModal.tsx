...
export default function CalibrationModal({ letter, onClose, onSuccess, variant = 'admin' }: CalibrationModalProps) {
  ...
  const startVisualization = () => {
    ...
    const visualize = () => {
      ...
      if (frameCount % 30 === 0) {
        ...
        // Default reduced 25% (12→9) for easier calibration
        const volumeThreshold = letter === 'o' ? 6 : (isNasal(letter) ? 3 : (isLiquid(letter) ? 6 : 9));
  ...
  const saveCalibrationToSupabase = async (
    ...
    try {
      ...
      // Create new snapshot with unique ID
      const newSnapshot = {
        id: `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, // Unique ID for deletion
        data: pattern,
        profileId: profileId,
        isNegative: false,
        score: 0,
