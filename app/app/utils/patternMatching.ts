...
// Helper function to get all global negatives from calibration data
function getGlobalNegatives(calibrationData: Record<string, CalibrationData>): Snapshot[] {
  const globalNegatives: Snapshot[] = [];

  Object.values(calibrationData).forEach(letterData => {
    if (letterData?.snapshots) {
      letterData.snapshots.forEach(snapshot => {
        if (snapshot.isNegative && snapshot.isGlobalNegative) {
          globalNegatives.push(snapshot);
        }
      });
    }
  });
...
export function matchAgainstLetter(
  ...
  // Get global negatives from ALL letters
  const globalNegatives = getGlobalNegatives(calibrationData);

  // Combine letter-specific snapshots with global negatives
  const allNegatives = [
    ...letterSnapshots.filter(s => s.isNegative),
    ...globalNegatives
  ];

  const positives = letterSnapshots.filter(s => !s.isNegative);

  // DEBUG: Log snapshot counts
  console.log(`🔍 Testing letter "${letter}": ${positives.length} pos, ${allNegatives.length} neg (${globalNegatives.length} global)`);
  ...
  // Test against all positive snapshots
  positives.forEach((snapshot, idx) => {
    if (!snapshot || !snapshot.data) return;

    const similarity = compareSnapshots(currentSnapshot, snapshot.data);
    console.log(`  POS[${idx}] similarity=${similarity.toFixed(1)}%`);
  ...
  // Test against all negative snapshots (letter-specific + global)
  allNegatives.forEach((snapshot, idx) => {
    if (!snapshot || !snapshot.data) return;

    const similarity = compareSnapshots(currentSnapshot, snapshot.data);
    const label = snapshot.isGlobalNegative ? 'GLOBAL-NEG' : 'NEG';
    console.log(`  ${label}[${idx}] similarity=${similarity.toFixed(1)}%`);
